...
export const ArchivedTablePage: React.FC = () => {
  ...
  return (
    <div className='mb-4'>
      <h4 className='py-2 text-center text-xl tracking-wide font-bold'>Archived Notes</h4>
      <Table>
        ...
        <tbody className="text-sm">
          {
            archivedNotes.map(note => (
              <tr key={`archived-${note.id}`} className="h-11 border-b hover:bg-gray-100">
                {
                  Object.values(note).slice(1, -1).map((info, index) => (
                    <Cell key={`archivedNoteValue-${index}`} elementType='td'>
                      {index === 1
                        ? convertDate(info)
                        : info
                      }
